...
Phase 17 — Testing (complete)

## NEXT ACTION

Phase 18 — Deployment: web deployment, database, worker, secrets, scheduler,
monitoring, backups, domain, production smoke test. Acceptance: production
health checks green.
...
- [x] Secrets server-side — all credentials are server-only env (`API_KEY`, `ADMIN_API_KEY`, `ODDS_API_KEY`, `PARLAY_API_KEY`, `DATABASE_URL`) with no `NEXT_PUBLIC_*` variants anywhere in `apps/web`; static test (`apps/web/lib/security/security.test.ts` "secrets server-side only") scans the app/lib/components/proxy source for `process.env` reads and asserts no NEXT_PUBLIC secret or unauthorized env variable is read, and no env read lives in `components/**`. A real ParlayAPI key was found committed during this phase; see the Phase 18 re-verification note in the Phase 16 evidence section for the corrected file attribution, the scanner hardening, and the still-open provider-side rotation. `scripts/secret-scan.ts` (`npm run security:scan`) gates CI against committed credentials
...
- **Open incident, re-verified in Phase 18.** A real ParlayAPI key was committed during Phase 3 exploration, and the value was subsequently written into this file's Phase 16 incident note and changelog entry. Phase 18 verification established: the value was published in `PROJECT_STATE.md` at `origin/main` (`84c63c0`) — **not** in `.env.example`, which carries only placeholders and did not exist in the earlier commits — and no commit in this repository's history contains a `PARLAY_API_KEY=<value>` assignment (the two SHAs previously cited here, `a92e064`/`7985e73`, never existed; the real history is `89069c5`, `35d5f69`, `bf3aa5b`, `84c63c0`). Both working-tree occurrences are now redacted; `.dockerignore` excludes root `*.md` so this file can never enter a Docker build context; and `scripts/secret-scan.ts` now matches the prose form (`ParlayAPI key (`…`)`) that previously slipped through — `npm run security:scan` reproduced the finding before the redaction and passes after, with a self-test that pins both the detection and the non-firing cases. **The credential must still be rotated/revoked at ParlayAPI by the user**, because it reached `origin`; purging published history is a separate destructive action that has not been taken. The scan reads `git ls-files --cached --others --exclude-standard`, so freshly added secrets fail CI before they can be pushed, but it cannot inspect already-published commits.
...
- [x] Unit
- [x] Settlement regression
- [x] Outcome-state regression
- [x] False-arb regression
- [x] Optimizer regression
- [x] Provider fixtures
- [x] Integration
- [x] E2E
- [x] Load/candidate-generation test

**Acceptance:** CI green.

Phase 17 gate results (evidence):

- New permanent regression fixtures: `tests/fixtures/regression/{cases,settlement,events,markets,load,index}.ts` — 13 golden candidate cases (incl. reciprocal traps that must never be classified from `sum(1/odds) < 1` alone, §2/§45), 20 settlement golden rows, 11 score lines, 13 market-normalization golden cases (label classifier + provider-native key registry, exact family/period/marketType/participant/line/via pins) and a seeded 20-event × 15-leg batch builder (`mulberry32`, per-event underround forcing on `arbEveryN`, BTTS pair underround on every event).
- New `tests/regression` workspace `@22void/regression` (registered in root `workspaces`, no selectable `tests/*` glob): suites `unit` (line math §12–§14, payout §10, freshness §38–§39), `settlement` (every golden row → exact result/components/return), `outcome-state` (state-model classes, both-win both-metrics legality, payoff matrix), `false-arb` (structure classification table, generator event isolation, EVENT_MISMATCH/STALE_ODDS pruning, scan lifecycle ARB/REJECTED/NO_ARB, reciprocal-trap proof), `optimizer` (LP vs classic closed forms, multiplier matrix = spec §22 A matrix, predictable rejection inputs), `provider-fixtures` (odds-api/parlay-api raw fixture translation incl. skipped-unregistered-market rule, canonical records, market-normalization golden table → valid line-aware `MarketStructure`), `integration` (normalize → persist → detect over `createMemoryWorkerStore`: VERIFIED_ARB with stable episode key, STALE via old source price, money-back state per cycle) and `load` (deterministic batch reproduction, underround forcing, per-event candidate enumeration inside the 20k cap without event merging, guaranteed-rate ARB density within a 30s budget).
- `npm run test:fixtures` = `npm run test --workspace @22void/regression`; root scripts single entry point.
- Gate green: lint 0 errors, `npm run typecheck` all workspaces clean, `npm run build` clean (incl. Next.js production build), `npm test` 560 passing across workspaces + 96 in the regression workspace, `npm run test:e2e -w @22void/web` 8/8 passing (landing smoke + dashboard overview/inspection/filters/rejection-explanation/event-matrix/providers flows), `npm run state:check` stays green.
